...
#     -N $JOB_NAME \
# #    -m bea -M $EMAIL \
#     -v SWEEP_CONFIG_FILE=$SWEEP_CONFIG_FILE,WORKDIR=$WORKDIR,SCRIPTDIR=$SCRIPTDIR \
#     -d $WORKDIR \
#     -t 1-$NUM_OF_JOBS \
#     $SCRIPTDIR/$EXECUTOR

export PBS_NUM_PPN=1
export WORKDIR SCRIPTDIR SWEEP_CONFIG_FILE
i=1
while [ $i -le $NUM_OF_RUNS ]; do
...
if [ $PORT -ne 0 ]; then
	$SCRIPTDIR/endSweep.sh -p $PORT -d $WORKDIR 
fi